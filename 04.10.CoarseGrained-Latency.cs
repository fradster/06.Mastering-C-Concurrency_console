/*
Coarse-grained approach - how does it influence latency.
...
static class DataShareEx
{
	...
	static void Main()
	{
		...
		for (int longThreadCount = 0; longThreadCount< numOfLongThreads; longThreadCount++)
		{
			...
			//for all threads created so far, add long calculation task
			for (int i = 0; i< longThreadCount; i++)
				longTasks.Add(Task.Factory.StartNew(() => SleepTask(1000)));
